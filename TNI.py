...
tf.flags.DEFINE_float('momentum', 0.8, 'momentum about the model.')
...
tf.flags.DEFINE_string('output_dir', './TNI',
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    x_nes1 = x - momentum * alpha * grad
    x_nes2 = x + momentum * alpha * grad

    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_v2, end_points_v2 = inception_resnet_v2.inception_resnet_v2(
    ...
    #
    cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v2)
    noise = tf.gradients(cross_entropy, x)[0]

    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_v3_1, end_points_v3 = inception_resnet_v2.inception_resnet_v2(
            x_nes1, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    cross_entropy_1 = tf.losses.softmax_cross_entropy(one_hot, logits_v3_1)
    noise += tf.gradients(cross_entropy_1, x)[0]

    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_v3_2, end_points_v3 = inception_resnet_v2.inception_resnet_v2(
            x_nes2, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
