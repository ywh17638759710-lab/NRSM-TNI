...
tf.flags.DEFINE_float('momentum', 1.6, 'momentum about the model.')

tf.flags.DEFINE_integer('number', 20, 'the number of images for variance tuning')

tf.flags.DEFINE_float('beta', 4.0, 'the bound for variance tuning.')
...
tf.flags.DEFINE_string('output_dir', './NRSM',
...
def batch_grad(x, one_hot, i, max_iter, alpha, grad):
    ...
    x_neighbor_nes_1 = x + alpha * momentum * grad + tf.random.uniform(x.shape, minval=-alpha, maxval=alpha)
    x_neighbor_nes_2 = x - alpha * momentum * grad + tf.random.uniform(x.shape, minval=-alpha, maxval=alpha)
    ...
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        ...
        grad_1 = tf.gradients(cross_entropy, x_neighbor)[0]

    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3_1, end_points_v3 = inception_v3.inception_v3(
            x_neighbor_nes_1, num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
        cross_entropy_1 = tf.losses.softmax_cross_entropy(one_hot, logits_v3_1)
        grad_2 = tf.gradients(cross_entropy_1, x_neighbor_nes_1)[0]

    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3_2, end_points_v3 = inception_v3.inception_v3(
            x_neighbor_nes_2, num_classes=1001, is_training=False, reuse=tf.AUTO_REUSE)
        cross_entropy_2 = tf.losses.softmax_cross_entropy(one_hot, logits_v3_2)
        grad_3 = tf.gradients(cross_entropy_2, x_neighbor_nes_2)[0]
    grad += (grad_1 + grad_2 + grad_3) / 3.0
    i = tf.add(i, 1)
    return x, one_hot, i, max_iter, alpha, grad

def graph(x, y, i, x_max, x_min, grad, variance, variance_last):
    ...
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v2, end_points_v2 = inception_v3.inception_v3(
            x, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)

    pred = tf.argmax(logits_v2, 1)  # resnet_v2
    ...
    _, _, _, _, _, noise = tf.while_loop(grad_finish, batch_grad, [x, one_hot, iter, max_iter, eps*FLAGS.beta,
                                                                         tf.zeros_like(grad)])

    noise = noise / (1. * FLAGS.number)

    # noise = tf.nn.depthwise_conv2d(noise, stack_kernel, strides=[1, 1, 1, 1], padding='SAME')

    noise = momentum * grad + (noise) / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)
    ...
    return x, y, i, x_max, x_min, noise, variance, variance_last


def stop(x, y, i, x_max, x_min, grad, variance, variacne_last):
    num_iter = FLAGS.num_iter
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        x_adv, _, _, _, _, _, _, _ =\
            tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min, tf.zeros(shape=batch_shape), tf.zeros(shape=batch_shape),
